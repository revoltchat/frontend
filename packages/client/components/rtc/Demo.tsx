...
import { Avatar, Button, Column, OverflowingText, Row, iconSize } from "@revolt/ui";
...
import { useTracks, useConnectionState, TrackLoop, useEnsureParticipant, useIsMuted, useTrackRefContext, useIsSpeaking, VideoTrack, isTrackReference, TrackReference } from "solid-livekit-components";
...
export function LeParticipant() {
  ...
  return <Tile speaking={isSpeaking() ? 'yes' : undefined}
  ...
  <Switch fallback={
    <div class={css({
      ...
      <Avatar fallback={Math.random().toString()[2]} size={64} interactive={false} />

    </div>
  }>
    <Match when={ isTrackReference(track) &&( track.publication?.kind === 'video' || track.source === Track.Source.Camera || track.source === Track.Source.ScreenShare)}>
      <VideoTrack style={{"grid-area": '1/1'}} trackRef={track as TrackReference} manageSubscription={true} />
    </Match>
    </Switch>
    <div class={css({
      minWidth: 0,
      ...
      <Show when={isMuted()}>
        <div class={css({ borderRadius: '100%', background: isTrackReference(track) ? '#0006' : '#0001', padding: '1' })}>
    ...
    >
      ...
      <Row justify="center">
        <Actions>
          ...
          >
            <Match when={voice.state() === "READY"}>
              <Button onPress={() => voice.connect()}>Join Call</Button>
            </Match>
          </Switch>
