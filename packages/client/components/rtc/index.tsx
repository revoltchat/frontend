...
import { RoomContext, AudioTrack, useTracks, useMaybeRoomContext } from "solid-livekit-components";
...
class Voice {
  ...
  async connect() {
    ...
    const name = `hello${Math.random().toString().substring(2, 8)}`;

    const resp = await fetch(
      `https://livekit-test-app.revolt.wtf/api/get-participant-token?room=frontend-test-room&username=${name}`
    );
    ...
    await room.connect("wss://livekit-test-node.revolt.wtf", data.token);
  }
...
export function RoomAudioManager() {
  ...
  const filteredTracks = createMemo(() => tracks().filter(track => !isLocal(track.participant) && track.publication.kind === Track.Kind.Audio));
...
export function VoiceContext(props: { children: JSX.Element }) {
  const [voice] = createSignal(new Voice());
